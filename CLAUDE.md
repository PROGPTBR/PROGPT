# Projeto: PROGPT — IA de Procurement (2B Supply)

## Contexto
Chatbot especialista + suite de **8 assistentes** que executam tarefas de Strategic Sourcing
(RFP/RFQ, Kraljic, Porter, Financial Score, Curva ABC, Perfil de Categoria, Negociação,
Supplier Scorecard) + **busca de fornecedores** (`/assistants/suppliers`, CNAE/região), treinado
em centenas de artigos sobre teorias, frameworks e práticas de procurement.

**Branding oficial fechado em sub-projeto 29 (2026-05-28)**:
- Produto: **PROGPT** (era ProcurementGPT internamente). Algumas chaves de cache/log mantêm
  `procurementgpt_*` como prefixo pra não invalidar estado de usuários existentes.
- Empresa responsável: **2B Supply** (CNPJ 36.335.299/0001-82)
- Contato comercial/DPO: comercial@2bsupply.com.br · +55 (21) 99979-2912
- Single source of truth: [lib/legal/constants.ts](lib/legal/constants.ts)

Audiência: gestores de compras brasileiros (PT-BR primário, EN secundário).

## Stack obrigatória
- Next.js 14 App Router + TypeScript strict
- Tailwind + shadcn/ui (tema light/dark via `next-themes`)
- Supabase (Postgres + pgvector + Auth + Storage)
- OpenAI SDK (`openai` v6) — **tier de geração (chat + assistant generate/refine/extract/negotiation) em `gpt-5.4-mini`** (sub-projeto 33, via `OPENAI_MODEL_GENERATION`); **tier de routing (classificador, condenser, follow-ups, título) em `gpt-4o-mini`** (barato, alto volume); `parsePdfMultimodal` (Responses API com `input_file` PDF base64; >20 MB usa Files API) no tier multimodal. Toda a stack LLM passou para OpenAI em 2026-05-08 (Gemini removido)
...
| 21-24 | `assistants-suite-complete` (consolidado — backfill) | Expansão do hub de assistentes além do RFP (migrations 0015-0023). **Company profile**: `profiles.logo` (0015) + dados da empresa do comprador (0016), injetados nos prompts + capa dos `.docx`/`.xlsx`. **6 assistentes novos** via o padrão compartilhado `buildAssistantHandler` (classify determinístico opcional → narrativa LLM → `.docx`/`.xlsx`, refine chat via `buildRefineSystemForType`, dispatch chart/xlsx/docx em `runs/[id]/*`): **Kraljic** (0017, matriz 2×2 + bubble chart + import xlsx), **Porter** (0018, 5 forças), **Financial Score** (0019, score 0-100 de saúde financeira + extração multimodal de balanço PDF), **Curva ABC** (0020, Pareto + import xlsx), **Perfil de Categoria** (0021 + `sessions_active_perfil` 0022), **Negociação** (0023, Strategy Builder JSON + simulador multi-turno + score). Cada tipo: rota dedicada `/api/assistants/<type>`, entrada no `AssistantType` union + CHECK constraint em `templates`. **Supplier Search** (`/assistants/suppliers`): busca CNAE/região (base externa Receita via `RECEITA_DATABASE_URL`) + classificador + export CSV; rotas `/api/suppliers/*`. **Transcrição de áudio** (`/api/transcribe`, Whisper) + **anexos no chat** (`/api/chat/attachments`, parse multimodal de PDF/DOCX). _Numeração 21-24 é backfill consolidado — estas entregas não foram logadas individualmente na tabela._ |
...
| 27 | `billing-asaas-complete` | Sub-projeto 2 do roadmap go-live B2C: monetização via Asaas hosted checkout. **Modelo**: Free (chat ilimitado + 1 execução lifetime por assistente, 7 totais) vs Pro R$ 99/mês (chat + assistentes ilimitados). Cartão recorrente + Pix Garantido. Cycle: monthly. Cancellation end-of-period. Spec em [docs/superpowers/specs/2026-05-27-billing-asaas-design.md](docs/superpowers/specs/2026-05-27-billing-asaas-design.md). **Camada de dados** (migration 0027): `subscriptions (user_id unique, asaas_customer_id, asaas_subscription_id, status, plan, payment_method, current_period_start/end, cancel_at_period_end, cancelled_at)` + `billing_webhook_events (asaas_event_id unique, event_type, payload, processed_at, error)` pra idempotência+audit. RLS: user lê só própria sub; webhook table sem policies (service-role). **Libs**: `lib/billing/asaas.ts` (REST client com `createCustomer`/`createSubscription`/`cancelSubscription`/`getSubscription`, AbortSignal 15s, retorna `invoiceUrl` hosted checkout); `lib/billing/subscription.ts` (`getActiveSubscription` considera `past_due` ativo até `current_period_end`; **`isPro` bypassa pra `profiles.role='admin'`** — sub-projeto 27.1); `lib/billing/quota.ts` (`canUseAssistant` = isPro OR count(assistant_runs) === 0; fail-closed em DB error); `lib/billing/handle-paywall.ts` (toast helper com action "Ver planos"); `lib/validators/cpf.ts` (checksum módulo 11). **Endpoints**: `POST /api/billing/checkout` (auth → CPF valid → race protection → createCustomer/Subscription Asaas com `billingType: 'UNDEFINED'` pra cobrir cartão+Pix com 1 fluxo → upsert subscriptions → retorna invoiceUrl), `POST /api/billing/webhook/asaas` (sem auth, valida header `asaas-access-token` vs `ASAAS_WEBHOOK_TOKEN`, idempotency via insert with on conflict do nothing, switch nos events: `PAYMENT_CONFIRMED`/`PAYMENT_RECEIVED` → active+set period; `PAYMENT_OVERDUE` → past_due; `PAYMENT_REFUNDED`/`PAYMENT_DELETED`/`SUBSCRIPTION_DELETED` → cancelled), `POST /api/billing/cancel` (auth → Asaas cancel → marca cancel_at_period_end=true, mantém Pro até fim do ciclo). **Paywall enforcement**: hook em `lib/assistants/handler.ts` antes de `createRun` chamando `canUseAssistant(user.id, config.type)` — retorna 402 `{ error: 'paywall', plan: 'free', assistant_type }`. Mesmo check no `/api/assistants/negotiation/strategy` (não usa o handler shared). Client trata via `handlePaywallResponse(res, type)` em todos os 7 XAssistant.tsx — toast.error com action "Ver planos" + reseta state machine. **UI**: `/pricing` (public; `<PricingTable>` com 2 colunas Free/Pro; CTA Pro abre modal `<CheckoutForm>` exigindo Nome+CPF — CPF descartado após o post, não persiste no nosso DB pra reduzir LGPD surface); `/account/billing` (server component, `requireUser`, lê subscription, `<SubscriptionPanel>` com badge status, datas, payment method, botão cancelar/reativar); `<PaywallModal>` (rich modal alternativo, não wirado em v1 — toast cobre). UserRow ganha link "Assinatura". Env novas: `ASAAS_API_KEY`, `ASAAS_API_URL` (sandbox→prod switch), `ASAAS_WEBHOOK_TOKEN`, `NEXT_PUBLIC_PRO_PRICE_BRL`. +46 vitest novos (cpf, asaas, subscription, quota, checkout, webhook, cancel). |
| 28 | `legal-docs-complete` | Sub-projeto 3 do roadmap go-live B2C: docs legais publicados antes de continuar tomando pagamento. **Migration 0028** adiciona `profiles.terms_accepted_at timestamptz` + `profiles.terms_version text` (backfill 'v0-pre-launch' pra users existentes). **`lib/legal/constants.ts`** centraliza `CURRENT_LEGAL_VERSION`, `LEGAL_LAST_UPDATED`, `LEGAL_CONTACT_EMAIL`. **3 páginas públicas**: `/termos` (12 cláusulas — aceitação, descrição do serviço, planos+cobrança incluindo CDC art. 49, conta, uso aceitável, propriedade intelectual dos outputs, limitação de responsabilidade, mudanças, encerramento, foro SP com ressalva CDC de foro do consumidor), `/privacidade` (12 seções LGPD-compliant — controlador, dados coletados explícitos por tabela DB, bases legais Art. 7º LGPD, operadores listados com país+propósito, retenção, direitos Art. 18, segurança, incidentes Art. 48, contato DPO), `/cookies` (essenciais vs não-essenciais — hoje só essenciais; gerenciamento via banner+browser). **`<LegalDocument>`** componente shared: header com voltar + título + versão+data, render ReactMarkdown+remarkGfm com prose-sm prose-invert, footer com cross-links. **`<CookieConsent>`** banner client-only: localStorage key `procurementgpt_cookie_consent_v1`, 2 botões (Aceitar tudo / Apenas essenciais), persiste choice+timestamp, monta no root layout. **`<LegalFooter>`** discreto com 4 links (Termos · Privacidade · Cookies · Suporte) — montado em `AuthShell` (login/signup/forgot/reset) + landing footer existente. **SignupForm** ganha checkbox obrigatório "Li e aceito os Termos de Uso e Política de Privacidade" linkando os docs (target=_blank); button disabled até marcar. **`/api/auth/signup`** body zod ganha `acceptedTerms: z.literal(true)` (server enforce mesmo que client trapaceie) + após signUp fire-and-forget UPDATE em `profiles` com `terms_accepted_at = now()` + `terms_version = CURRENT_LEGAL_VERSION`. +8 vitest (CookieConsent localStorage; SignupForm terms gate; signup endpoint terms persist). |
| 29 | `branding-progpt-complete` | Sub-projetos 4+6 do roadmap go-live B2C combinados: branding oficial **PROGPT** (2B Supply) + refresh da landing page. **`lib/legal/constants.ts`** vira fonte única da verdade: `PRODUCT_NAME='PROGPT'`, `COMPANY_NAME='2B Supply'`, `COMPANY_CNPJ='36.335.299/0001-82'`, `LEGAL_CONTACT_EMAIL='comercial@2bsupply.com.br'`, `LEGAL_CONTACT_PHONE='+55 (21) 99979-2912'`, `LEGAL_CONTACT_PHONE_TEL='+5521999792912'`. `CURRENT_LEGAL_VERSION` bumped pra `v2-2026-05-28` (alteração material em docs legais — preencheu CNPJ real). **/termos e /privacidade** atualizados (sumiu `[CNPJ A CADASTRAR]`; controlador = 2B Supply com CNPJ). **Landing page** refresh: NAV inclui Planos linkando /pricing (em vez de #beneficios), FEATURE_CARDS atualiza copy ("7 assistentes" em vez de "RFP+Kraljic+seis em breve"), nova seção `<section id="planos">` com pricing Free vs Pro (R$ 99/mês) substituindo a antiga "Por convite via 2B Supply" (signup self-service desde sub-projeto 25), FAQ atualizado pra mencionar PROGPT + planos + cancelamento, footer rebuilt em 3 colunas (BrandLogo+pitch · Navegação · Contato com phone+email+CNPJ+link 2bsupply.com.br) + barra inferior com © + links Termos/Privacidade/Cookies. **Metadata** em `app/layout.tsx` ganha title + description SEO. **Chrome update**: AuthShell footer ("PROGPT · uma plataforma 2B Supply"), Header mobile label ("PROGPT"), FEEDBACK_MAILTO aponta pra `comercial@2bsupply.com.br?subject=PROGPT%20feedback` (era `rgoalves@gmail.com`), prompt builder do RAG ("Ferramentas dedicadas do PROGPT"), Asaas subscription `description: 'PROGPT Pro · R$ 99/mês'` (aparece na fatura do cliente), `User-Agent: PROGPT/1.0` nas calls Asaas, `wb.creator='PROGPT'` em todos os .xlsx + `creator: 'PROGPT'` em todos os .docx gerados. Chaves de cache/log com prefix `procurementgpt_*` mantidas pra não invalidar estado de usuários existentes (localStorage cookie consent, etc.). |
| 30 | `email-transacional-complete` | Sub-projeto 8 do roadmap go-live B2C: 4 emails transacionais via Resend. **Migration 0029** adiciona `profiles.welcome_email_sent_at` pra idempotência do welcome (lock-then-send pra evitar double-send em retry de PKCE). **`lib/email/client.ts`** — wrapper Resend fail-soft (mesmo padrão de `recordApiUsage`): se `RESEND_API_KEY` missing ou Resend retorna erro/throw, retorna `{ok:false}` e segue. Forward de `idempotencyKey` pro Resend (24h dedup). **`lib/email/templates.ts`** — 4 templates HTML inline (zero deps): `buildWelcomeEmail` (CTA /chat), `buildPaymentConfirmedEmail` (recibo com valor BRL + próxima cobrança + CTA /account/billing), `buildPaymentOverdueEmail` (aviso de cartão recusado + cutoff date + CTA atualizar), `buildSubscriptionCancelledEmail` (confirma cancel + access until + CTA reativar /pricing). Shell compartilhada com logo PROGPT + brand color + footer com CNPJ 2B Supply + email/phone. **Hooks**: welcome em `/auth/callback` (acquire lock via `welcome_email_sent_at IS NULL` UPDATE → send se acquired); payment_confirmed + payment_overdue no webhook Asaas (busca email via `auth.admin.getUserById(sub.user_id)`); subscription_cancelled em `/api/billing/cancel`. Todos fire-and-forget. Idempotency keys: `welcome:{userId}`, `paid:{eventId}`, `overdue:{eventId}`, `cancel:{userId}:{ts}`. Env novas: `RESEND_API_KEY`, `EMAIL_FROM` (default 'PROGPT <onboarding@resend.dev>' funciona sem DNS; prod usa 'PROGPT <noreply@2bsupply.com.br>'), `APP_URL` (default Railway). +9 vitest novos (client fail-soft, idempotencyKey forwarding, 4 template shapes). |

| 31 | `scorecard-complete` | Supplier Scorecard (Strategic Sourcing passo 8) — #1 do ranking esforço×impacto (`docs/product/assistants-ranking.xlsx`). Espelha o arquétipo Kraljic via `buildAssistantHandler`. `lib/assistants/scorecard.ts` `scoreSuppliers` determinístico (peso normalizado → `weightedScore` 0-100, ranking estável em empate, faixas `estrategico`/`desenvolvimento`/`saida` por threshold padrão **70/40** configurável). Gráfico de ranking (barras horizontais, `@napi-rs/canvas`, `scorecard-chart.ts`), import `.xlsx` (grade Fornecedor×critérios, de-dup de ids de critério, `scorecard-import.ts`) + export `.xlsx` multi-sheet (Scorecard+Ranking+Gráfico, `scorecard-xlsx.ts`) + `.docx` com chart embutido, refine chat. Critérios editáveis+custom no form (grade dinâmica fornecedor×critério). Migration 0031 (CHECK +`scorecard`) + template default semeado. Spec/plano em `docs/superpowers/{specs,plans}/2026-05-29-supplier-scorecard*`. PR #98. +~37 vitest. |
...
| 33 | `chat-depth-gpt54mini` | Respostas do chat **rasas vs a profundidade da base** → o gargalo era o **modelo** (gpt-4o-mini sub-executava o SYSTEM_PROMPT sênior; eval:gen baseline ~3.50/5, 50% pass). **Upgrade do tier de geração pra `gpt-5.4-mini`** via `OPENAI_MODEL_GENERATION` (chat + assistentes; routing/classify/condense/followups continuam no gpt-4o-mini barato). eval:gen pós: **4.25/5, 92% pass** (cases de *aplicação prática* foram de 3.0/fail → 4-5; `structure` 5.0; TTFT **0.86s @ ~25k tok**; `temperature:0.3` aceito). Complementos: **contexto 8 → 12 trechos** (`RERANK_TOP_N` em `lib/rag/index.ts`; recall@5 segue 0.97) + **rebalanceamento do SYSTEM_PROMPT** (deixa explícito que "2-3 linhas" é abertura, não a resposta inteira — mantém byte-estável todas as frases travadas pelos 33 testes do prompt-builder). **Rate card** gpt-5.4-mini/5.4/5.5 em `lib/observability/api-usage.ts` ($0.75/$0.075/$4.50 por 1M no mini) adicionada ANTES do flip (fallback agora é gpt-5.5 — nunca sub-fatura). +5 vitest de custo. Flip de PROD = setar `OPENAI_MODEL_GENERATION` no Railway. |
| 34 | `negotiation-voice-mode` | **Modo voz no simulador de negociação** ("ensaie a reunião antes da reunião" — caso de uso descrito espontaneamente por prospect real, SAGRES). 3 capacidades sobre o simulador existente: **(a) Voz de saída** — `POST /api/assistants/runs/[id]/speak` sintetiza a fala do fornecedor via **`gpt-4o-mini-tts`** (steerable) na voz da persona (`lib/assistants/negotiation/voice.ts`: agressivo→onyx firme, colaborativo→nova caloroso, etc.; persona resolvida server-side do run, nunca do body); playback no `NegotiationChat` com toggle default-ON (localStorage `progpt_negotiation_voice_v1`), opener falado (autoplay fail-soft), botão "ouvir/parar" por bolha. **(b) Coach inline** — `POST /api/assistants/runs/[id]/advise` (`prompt-coach.ts`): timeout tático onde o LLM (tier generation) vira consultor DO COMPRADOR; bolha violeta distinta; mensagens `kind:'coach'` **filtradas client-side** do payload do `/negotiate` e do transcript do score (o /close lê transcript do DB, que nunca as recebe). Texto do composer vira a pergunta opcional. **(c) Anexos** — clipe no chat da negociação reusa `/api/chat/attachments` + `wrapWithAttachment` (ChatSession) pra subir contrato/proposta; `NegotiationTurnRequestSchema` content max 8000→16000 pra caber o `<anexo>`. Ambos endpoints novos: owner-gate (`getRunForOwner`) + `checkChatRateLimit`. **Rate card** TTS em `api-usage.ts` (ops `assistant-negotiation-speak`/`-advise`; $0.60/1M texto + ~$0.015/min áudio estimado por chars/600 — superestima, nunca sub-fatura). Fase 2 registrada (não construída): voz-a-voz realtime via `gpt-realtime-mini`. +16 vitest. |
| 35 | `chat-voice-realtime` | **Assistente de voz em tempo real no chat** ("converse com o comprador sobre qualquer tema da base + anexos" — generaliza o conceito do sub-projeto 34). **OpenAI Realtime API (`gpt-realtime-mini`) via WebRTC direto browser→OpenAI** com token efêmero: `POST /api/chat/voice/session` (Node, requireUser + rate-limit do chat) minta em `/v1/realtime/client_secrets` (TTL 10 min, `OpenAI-Safety-Identifier` = UUID pseudonimizado, LGPD) — **sem proxy de áudio no Railway**. **Base RAG por tool calling**: a sessão declara `buscar_base_conhecimento`; o browser executa via `POST /api/chat/voice/retrieve` (retrieve+rerank top-8 direto, pula classifier/condenser; fail-soft → contexto vazio) e devolve `function_call_output`. **Anexos**: parsedText do `/api/chat/attachments` vira `conversation.item.create` (contexto da conversa). Persona falada em `lib/voice/realtime-config.ts` (`buildVoiceInstructions` — respostas 2-4 frases, sem markdown, fundamento obrigatório via tool, não inventa autor/data; NÃO compartilha bytes com o SYSTEM_PROMPT do chat). Client: `hooks/useRealtimeVoice.ts` (WebRTC + data channel; tool-exec, transcripts, barge-in via server VAD, mute, timer 10 min) + `lib/voice/event-reducer.ts` (puro/testável, aceita nomes de evento GA E beta) + `components/chat/VoiceMode.tsx` (overlay: orb de status, transcript ao vivo, anexar, mutar, encerrar). Entrada: botão AudioLines no Composer (hero+inline) — **opt-in, nunca default** (custo ~$0.15-0.25/10 min). Ao encerrar, o transcript **vira histórico da sessão** (setMessages + onMessagesChange — continuidade voz↔texto). **Usage CLIENT-REPORTED** (acumulado dos `response.done`, postado em `POST /api/chat/voice/usage` + sendBeacon no unload; sem proxy não há fonte server-side — mitigado por caps no zod + TTL + rate-limit; op `chat-voice-realtime` com split por modalidade no metadata; sem split, fatura tudo como áudio — conservador). Mint validado ao vivo (HTTP 200, ek_) antes do build. +24 vitest. **Pós-deploy (feedback 2026-06-10)**: reconhecimento de fala — transcrição `whisper-1`→`gpt-4o-mini-transcribe` + prompt de jargão de procurement, `server_vad`→`semantic_vad`, `noise_reduction: near_field` + constraints de mic (echo/noise/AGC) (PR #124). **Escolha de voz**: catálogo `VOICE_OPTIONS` (6 vozes, todas validadas no mint), seletor no header do `VoiceMode` (localStorage `progpt_chat_voice_v1`), client manda `{voice}` no `POST /session`, **validação server-side via `isVoiceName`** (string fora do catálogo cai no `DEFAULT_VOICE='echo'`); troca em sessão ao vivo reconecta preservando o transcript (turnos arquivados no overlay — a Realtime API não troca voz após o modelo falar). |
| 36 | `fiscal-integration` | **Integração de dados fiscais brasileiros** via o servidor open-source **mcp-fiscal-brasil** (Python/FastAPI, MIT, sem API keys — BrasilAPI/ReceitaWS/SEFAZ) rodando como **serviço REST separado no Railway** (Docker; serverless OFF). PROGPT consome por HTTP, **sem cliente MCP**. **Fase 0** — `lib/fiscal/{client,types,cache}.ts` (wrapper fail-soft padrão `asaas.ts`: `consultarCnpj`/`riskScoreSupplier`/`analyzeCnpjCompliance`/`compareTaxRegimes`; `FISCAL_API_URL` vazio ⇒ tudo off; **prefixa `https://` se faltar esquema**; retry 1x p/ cold start; cache 24h por CNPJ; contrato em `docs/product/fiscal-api-contract.md`). **Fase 1** — assistente **Homologação de Fornecedor** (`type='homologacao'`, migration 0032 + template seed; `buildAssistantHandler` com `classify.run` agora **async** — fetch fiscal fail-soft; CNPJ → situação cadastral, score de risco, compliance, **cadastro completo: endereço + QSA/sócios + natureza + capital**; relatório `.docx`; histórico/CTA/Hub/exemplo WEG wirados de cara). **+3 enriquecimentos**: (A) **links de certidões** oficiais por UF (`certidoes-links.ts`); (B) **sanções CEIS/CNEP** (Portal da Transparência, `lib/fiscal/sancoes.ts`, env `PORTAL_TRANSPARENCIA_TOKEN`, achado = ⛔ impeditivo) — **em espera, desligado por padrão**; (C) **due diligence reputacional** via **web search da OpenAI** (`lib/fiscal/reputacao.ts`, Responses API tool `web_search`, rotulado indicativo/não-oficial, `HOMOLOGACAO_WEBSEARCH=false` desliga). **Fase 2** — Análise Financeira: campo CNPJ vira consulta real (`fetchFiscalSnapshot` + `POST /api/fiscal/lookup` p/ o botão "Consultar"; bloco `<dados-fiscais>` no prompt — situação ≠ ATIVA é alerta grave). **Fase 3** — Busca de Fornecedores: botão "Verificar situação fiscal" enriquece os visíveis (até 30, `POST /api/suppliers/enrich`, cap de concorrência — serviço sem lote) com selo cadastral+risco por card. **Fase 4** — tool de voz `consultar_dados_fiscais` (`POST /api/chat/voice/fiscal`) no assistente realtime (mint revalidado ao vivo com 2 tools). Tudo **fail-soft** (serviço fora ⇒ features dormentes). Validado ao vivo (PAINCO/WEG). Envs novas: `FISCAL_API_URL`, `PORTAL_TRANSPARENCIA_TOKEN`, `HOMOLOGACAO_WEBSEARCH`. PRs #133-144. +~60 vitest. |
| 37 | `govdata-compras-publicas` | **Dados de compras públicas brasileiras** via o MCP open-source **mcp-brasil** (533 tools / 70 fontes). NÃO rodamos o servidor nem cliente MCP — consumimos as **APIs públicas subjacentes direto por HTTP, sem chave** (padrão fail-soft de `lib/fiscal`). `lib/govdata/{client,types,cache}.ts`: `govGet(base, path, params)` p/ 3 bases (PNCP `pncp.gov.br/api/consulta`, Compras `dadosabertos.compras.gov.br`, BACEN `api.bcb.gov.br/dados`) com default público + override por env; timeout 20s, 1 retry em 5xx/timeout, **default-ON** (`GOVDATA_ENABLED=false` desliga). Contrato em `docs/product/govdata-api-contract.md`. **Fase 1 (o prêmio)** — assistente **Pesquisa de Preços** (`type='pesquisa_precos'`, migration 0036 + seed): `lib/govdata/precos.ts` `buscarCatmat` resolve texto→CATMAT navegando Classe→PDM→Item com 3 picks do LLM (o catálogo **não tem busca textual** — `descricaoItem` retorna 0), `precoReferencia` puxa preços praticados (`/modulo-pesquisa-preco/1_consultarMaterial?codigoItemCatalogo=`) e calcula estatística **robusta** (mediana + IQR + descarte de outliers); `lib/assistants/pesquisa-precos.ts` via `buildAssistantHandler` (mapa de preços `.docx`, histórico/CTA/Hub/exemplo wirados de cara — a regex de detecção do CTA passou a aceitar underscore). Validado ao vivo: "açúcar refinado" → mediana R$ 4,48/kg de 465 compras. **Fase 2** — homologação ganha bloco `<historico-publico>` (`lib/govdata/fornecedor.ts` `historicoPublico` via `/modulo-contratacoes/3_consultarResultadoItensContratacoes_PNCP_14133?niFornecedor=`, janela ≤365d) = "fornece pro governo" como sinal positivo de idoneidade. **Fase 3** — indicadores econômicos (`lib/govdata/indicadores.ts`: Selic 432, IPCA acum.12m 433, dólar 1; `accumulate12m` puro): tool de voz `consultar_indicadores_economicos` (`POST /api/chat/voice/indicadores`, mint revalidado ao vivo com 3 tools) + bloco macro opcional na Análise Financeira + **dashboard `/assistants/indicadores`** (painel ao vivo de 6 indicadores — Selic/CDI/IPCA/IGP-M/dólar/euro — com sparkline SVG sem dep + "leitura para compras" via IA; `painelIndicadores`, `GET /api/govdata/indicadores`, `POST /api/assistants/indicadores/leitura`; séries de nível via **range query** porque `ultimos/N` é capado p/ séries diárias; é um **viewer** — sem `buildAssistantHandler`/migration/paywall — registrado no hub+CTA como 11ª ferramenta; PR #157). Tudo **fail-soft**. Envs novas: `GOVDATA_ENABLED`, `PNCP_API_URL`, `COMPRAS_API_URL`, `BACEN_API_URL` (todas opcionais, default público). PRs #147/#150/#154/#155/#156/#157. +~62 vitest. _Follow-ups: injeção de indicadores no assistente de Negociação; selo "fornece pro governo" na busca de fornecedores._ |
...
**Test count atual:** 1102 vitest (1095 verdes; 7 de billing/signup pré-quebrados na `main` por PRs de outros devs — guard de produção do Asaas removido), 28 pytest, typecheck zero erros. CI gate: `recall@5 ≥ 0.85` em PR + push main.

## Estrutura de pastas

> Árvore abaixo é do núcleo RAG/chat/admin (Milestone 1). Dirs adicionados depois e
...
/supabase/migrations
  ...
  00000000000014_templates_and_assistant_runs.sql (templates + assistant_runs, sub-projeto 20)
  00000000000015..0016                   (profiles.logo + company profile fields)
  00000000000017..0021                   (assistant_type CHECK: kraljic/porter/financial/abc/profile)
  00000000000022_sessions_active_perfil.sql · 0023_negotiation_assistant_type.sql
  00000000000024_api_usage_user_id.sql · 0025_rate_limit_anon.sql · 0026_security_hardening.sql
  00000000000027_billing.sql · 0028_legal_acceptance.sql · 0029_email_idempotency.sql
  00000000000030_admin_funnel.sql        (admin_funnel_metrics fn — /admin/funnel)
  00000000000031_scorecard_assistant_type.sql (CHECK +scorecard)
/.github/workflows
...
GOVDATA_ENABLED=               # sub-projeto 37 — kill-switch dos dados de compras públicas (PNCP/Compras/BACEN). Default ON (APIs públicas grátis); 'false' desliga Pesquisa de Preços, histórico público e indicadores
PNCP_API_URL=                  # sub-projeto 37 — opcional; default https://pncp.gov.br/api/consulta
COMPRAS_API_URL=               # sub-projeto 37 — opcional; default https://dadosabertos.compras.gov.br
BACEN_API_URL=                 # sub-projeto 37 — opcional; default https://api.bcb.gov.br/dados
```
...
## O que evitar
- Adicionar um assistente novo (tile em `/assistants/*`) sem registrá-lo no **CTA do chat** — atualize `AssistantToolType` + `META` + `VALID_TYPES` + `STRIP_TYPES` em [components/chat/AssistantToolCTA.tsx](components/chat/AssistantToolCTA.tsx) **E** a lista "Ferramentas dedicadas" + o mapa tema→ferramenta no `SYSTEM_PROMPT` ([lib/rag/prompt-builder.ts](lib/rag/prompt-builder.ts)). Sem isso o chat não sugere o assistente nem mostra o card (o `scorecard` do sub-projeto 31 ficou de fora até 2026-06-02). O card é detectado client-side do conteúdo (`detectAssistantToolCTA`) e o caminho cru é removido do texto exibido (`stripAssistantPaths`) — não depende mais da annotation efêmera do backend. **Tipo com underscore** (ex.: `pesquisa_precos`): a regex de `detectAssistantToolCTA`/`STRIP_TYPES` precisa aceitar `_` (`[a-z0-9_-]`) — senão o caminho é truncado e o card some.
- Tentar resolver descrição livre → código CATMAT via busca textual do Compras.gov.br (`/modulo-material/4_consultarItemMaterial?descricaoItem=`) — **o catálogo NÃO tem busca textual** (retorna 0 pra qualquer termo). A resolução em `lib/govdata/precos.ts` (`buscarCatmat`) navega a hierarquia Classe→PDM→Item com 3 picks do LLM. Os preços (`/modulo-pesquisa-preco/1_consultarMaterial`) exigem `codigoItemCatalogo`; não há busca de preço por texto. Ver `docs/product/govdata-api-contract.md`.
- Assumir que `lib/govdata/*` está atrás de um serviço/env como o fiscal — é **default-ON** (APIs públicas grátis, defaults hardcoded). `GOVDATA_ENABLED=false` é o kill-switch; `PNCP_API_URL`/`COMPRAS_API_URL`/`BACEN_API_URL` só pra apontar pra proxy. Não confundir com `FISCAL_API_URL` (esse sim desliga tudo quando vazio).
- Setar `OPENAI_MODEL_<TIER>` pra um modelo **sem rate card** em `lib/observability/api-usage.ts` — `computeCostUsdCents` resolve a tarifa por prefixo de modelo (`openAiRate`); carded hoje: `gpt-4o-mini`, `gpt-4o`, `gpt-5.4-mini`, `gpt-5.4`, `gpt-5.5`. Modelo não-carded cai no `RATE_FALLBACK` (= o mais caro conhecido, hoje gpt-5.5 — sobre-fatura, **nunca** sub-fatura). Ao adotar um modelo novo, **adicione a tarifa em `OPENAI_RATES` + branch em `openAiRate` ANTES** do flip do env. Pra validar a qualidade de um flip do tier `generation`, rode `npm run eval:gen --model <candidato>` e compare (baseline gpt-4o-mini ~3.50-3.92/5; gpt-5.4-mini ~4.25/5) — o gate de CI `recall@5` é só retrieval e NÃO pega regressão de geração.
...
- Conectar nos scripts via host direto `db.<ref>.supabase.co:5432` — é **IPv6-only** e falha em rede sem IPv6 ("Socket is not connected"). Use `scripts/db_connect.py` `connect()` (tenta direto → cai no **pooler IPv4** `aws-1-us-west-2.pooler.supabase.com:5432`, user `postgres.<ref>`). Projeto está em **us-west-2**. `apply_migrations.py` já usa o helper; migrations one-shot devem usar também.
