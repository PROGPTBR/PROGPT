...
|---|------|-----------|---------|
| 5 | **Webhook Asaas dropa evento desconhecido em silêncio + sem alerta de falha** | `app/api/billing/webhook/asaas/route.ts` | Evento perdido / 500 nas 3 retries deixa user em estado errado (pago-mas-não-Pro, ou cancelado-mas-Pro) sem ninguém saber. Logar `warn` em evento desconhecido + alertar em falha (cron que varre `processed_at IS NULL` > 1h). |
| 6 | **Turn-leak no assistente de negociação** | `app/api/assistants/runs/[id]/{negotiate,setup,close}` | Rate-limited + owner-checked, mas sem `canUseAssistant`. Não é "assistentes free ilimitados" (criar run é gated), mas um free que gastou seu 1 run lifetime pode mandar turnos ilimitados dentro dele. Adicionar check de paywall. |
