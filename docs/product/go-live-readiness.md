...
| 8 | **Sem analytics de produto / funil de conversão** | ✅ PR #93 + migration aplicada | Dashboard interno `/admin/funnel` (sem vendor externo, sem LGPD nova): funil signup→ativação→pago + uso por assistente, via SQL function `admin_funnel_metrics()` (migration 0030, aplicada 2026-05-29 via pooler us-west-2). Já retorna dados reais. |
