  ...
  vi.doMock('@/lib/assistants/templates', () => ({
    getTemplate: vi.fn().mockResolvedValue(
      templateBody
        ? {
            id: 'tpl-1',
            assistant_type: 'scorecard',
            name: 'Scorecard Padrão',
            description: null,
...
function buildPost(body: unknown): Request {
  return new Request('http://x/api/assistants/scorecard', {
    method: 'POST',
...
const validBody = {
  ...
  params: {
    ...
    suppliers: [{ name: 'A', segment: '', scores: { q: 8 } }],
    thresholds: { strategic: 70, development: 40 },
  },
};

describe('POST /api/assistants/scorecard', () => {
  it('returns 401 when unauthenticated', async () => {
    setupMocks({ authed: false });
    const { POST } = await import('@/app/api/assistants/scorecard/route');
