  ...
  vi.doMock('@/lib/assistants/templates', () => ({
    getTemplate: vi.fn().mockResolvedValue(
      templateBody
        ? {
            id: 'tpl-1',
            assistant_type: 'diagnostico_aquisicao',
...
function buildPost(body: unknown): Request {
  return new Request('http://x/api/assistants/diagnostico_aquisicao', {
    method: 'POST',
...
describe('POST /api/assistants/diagnostico_aquisicao', () => {
  it('returns 401 when unauthenticated', async () => {
    setupMocks({ authed: false });
    const { POST } = await import('@/app/api/assistants/diagnostico_aquisicao/route');
    const res = await POST(buildPost(validBody));
    expect(res.status).toBe(401);
  });

  it('returns 400 for invalid body (missing descricaoCompra)', async () => {
    setupMocks();
    const { POST } = await import('@/app/api/assistants/diagnostico_aquisicao/route');
    const res = await POST(
      buildPost({
        templateId: '00000000-0000-0000-0000-000000000001',
        params: { ...validBody.params, descricaoCompra: '' },
      }),
    );
  ...
  it('returns 400 for invalid templateId (not uuid)', async () => {
    setupMocks();
    const { POST } = await import('@/app/api/assistants/diagnostico_aquisicao/route');
    const res = await POST(buildPost({ ...validBody, templateId: 'not-a-uuid' }));
    expect(res.status).toBe(400);
  });
