...
import type { SearchResponse, UF } from '@/lib/suppliers/types';
...
export function SuppliersResults({
  ...
  const filtered = useMemo(() => {
    return response.suppliers.filter((s) => {
      if (sizeFilter === 'small_plus' && s.porte === 'ME') return false;
      if (sizeFilter === 'medium_plus' && s.porte !== 'DEMAIS') return false;
      if (contactOnly && !s.telefone && !s.email) return false;
      return true;
    });
  }, [response.suppliers, sizeFilter, contactOnly]);
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {totalLabel} {response.total === 1 ? 'resultado' : 'resultados'}
              {response.total >= 500 ? ' (limitado a 500 — refine para ver mais)' : ''}
            </p>
          </div>
      ...
      {response.suppliers.length > 0 && (
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card p-3">
          ...
          <div className="ml-auto text-xs text-muted-foreground">
            Mostrando {filtered.length} de {response.suppliers.length}
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((s) => (
            <SuppliersResultCard key={s.cnpj} supplier={s} />
          ))}
        </div>
      )}
    </div>
