"""One-shot: insert the default Porter (5 Forces) template.

Idempotent: if a template with the same name + assistant_type='porter'
...
TEMPLATE_DESCRIPTION = (
    'Template padrão para análise das 5 Forças de Porter (1979). '
    'Inclui sumário executivo, seção por força (rivalidade, novos entrantes, '
    'substitutos, poder dos fornecedores, poder dos compradores), síntese '
    'estratégica em tabela e recomendações para o comprador.'
)
BODY_PATH = 'docs/product/templates/porter-padrao.md'
...
with psycopg.connect(dsn, autocommit=True) as conn:
    with conn.cursor() as cur:
        cur.execute(
            ...
            "where assistant_type='porter' and name=%s",
            (TEMPLATE_NAME,),
        )
