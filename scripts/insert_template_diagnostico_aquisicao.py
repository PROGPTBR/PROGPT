"""One-shot: insert the default Diagnóstico de Aquisição template into the
templates table.

Idempotent: if a template with the same name + assistant_type=
'diagnostico_aquisicao' already exists, this updates body_md/description
...
TEMPLATE_DESCRIPTION = (
    'Template padrão para o Diagnóstico de Aquisição (CAPEX/OPEX → KPIs → '
    'SOURCE/CONTRACT/BUY). Inclui resumo executivo + diagnóstico + estratégia '
    'recomendada + KPIs + apêndice com metodologia e confidencialidade.'
)
BODY_PATH = 'docs/product/templates/diagnostico-aquisicao-padrao.md'
...
with psycopg.connect(dsn, autocommit=True) as conn:
    with conn.cursor() as cur:
        cur.execute(
            ...
            "where assistant_type='diagnostico_aquisicao' and name=%s",
            (TEMPLATE_NAME,),
        )
        ...
        if existing:
            ...
            cur.execute(
                ...
                values ('diagnostico_aquisicao', %s, %s, %s, %s)
                returning id
                """,
        ...
        cur.execute(
            ...
            "from templates where assistant_type='diagnostico_aquisicao' order by created_at desc"
        )
        print('\nTemplates Diagnóstico de Aquisição em produção:')
