"""One-shot: insert the default Financial Health Analyzer template.

Idempotent: if a template with the same name + assistant_type='financial'
...
TEMPLATE_DESCRIPTION = (
    'Template padrão para análise financeira de fornecedor. '
    'Score determinístico 0-100 a partir de 4 pilares ponderados '
    '(Liquidez 30%, Dívida/EBITDA 30%, Margem EBITDA 20%, ROE 20%) + '
    'narrativa com 12 indicadores, recomendação de compra (buy/caution/'
    'do_not_buy), termos de pagamento sugeridos e análise de risco de '
    'falência.'
)
BODY_PATH = 'docs/product/templates/financial-padrao.md'
...
with psycopg.connect(dsn, autocommit=True) as conn:
    with conn.cursor() as cur:
        cur.execute(
            ...
            "where assistant_type='financial' and name=%s",
            (TEMPLATE_NAME,),
        )
